...
int main(){
    ...
    C2 = 1.0 - beta / y_init;
    std::cout << t << " " << y_init << " " << std::log(std::abs(y - beta / (1.0 - C2 * std::exp(- alpha * t))) / std::abs(beta / (1.0 - C2 * std::exp(- alpha * t)))) << std::endl;
    ...
    for(int i=1; t<T_limit; i++){
        ...
        t = i * dt;
        //t秒後のときのyの値を表示する．
        std::cout << t << " " << y << " "<< std::log(std::abs(y - beta / (1.0 - C2 * std::exp(- alpha * t))) / std::abs(beta / (1.0 - C2 * std::exp(- alpha * t)))) << std::endl;
    }
}
